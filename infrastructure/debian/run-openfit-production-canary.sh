...
jq -e --arg image "$IMAGE" --arg imageSha256 "$expected_image_sha" --argjson imageBytes "$expected_image_bytes" '.format == "home-lab-debian-openfit-canary-transfer-v1" and .image == $image and .imageSha256 == $imageSha256 and .imageBytes == $imageBytes and (.imageId | test("^sha256:[0-9a-f]{64}$")) and .backupReplicaCount == 2 and (.backupSha256 | test("^[0-9a-f]{64}$")) and (.backupSampleSha256 | test("^[0-9a-f]{64}$")) and .backupBytes >= 1000000000 and .backupAgeSeconds >= 0 and .backupAgeSeconds <= 604800 and .backupValidation == "existing-local-replicas-sidecar-and-sample" and .backupActualFullHashRecomputed == false and .openfitStoppedForSnapshot == false and .privateDataExported == false and .s3Checked == false' <<< "$transfer_marker" >/dev/null || fail "canary transfer marker differs"
