...
There is no independent Proton timer. The native
[unit configuration path](../docs/operations.md#backup-unit-definitions) owns all nine
existing definitions only; it neither bootstraps backups nor changes enabled/active
states. Legacy activation still requires the evidence below. The expanded native
scope passed a zero-change live preview; no expanded apply was performed.
The [September 14 observation](../docs/operations.md#observed-baseline--2026-09-14)
confirmed enabled schedules and successful daily unit exits, not snapshot integrity
or restore readiness. The monthly Proton failure was reproduced as a mount-namespace
traversal defect; see the [unit fix and validation limits](../docs/operations.md#proton-maintenance-unit).
A successful mount probe is not a full maintenance/restore result. `restic-proton` stays UID/GID **60000**,
without login, supplementary groups or production-tree access. Native
[account configuration](../docs/operations.md#confined-backup-account) requires the
existing fixed identity; it does not create accounts or renumber ownership.
...
backup scope. The September 15 (PDT) native
[runtime-file slice](../docs/operations.md#same-content-backup-runtime-files) adopts
same-content runner/input metadata only; source review and the full live preview
passed with zero changes. No apply was performed. It refuses content drift and
untrusted file permissions; policy/scoping and retained
journals are not migrated. Future content changes require coordinated policy/journal
review, not receipt regeneration. This adds no backup-health or recovery proof.
...
Native host variables declare these same pins for existing-host adoption. Source
review and the combined live preview passed with zero changes; no reinstall was
performed or validated. The native path requires installed runtime-policy tool
equality before replacement; it never
bootstraps or rewrites policy. Future upgrades need coordinated policy/bundle
inputs and Proton qualification, not independent pin bumps. This adds no recovery
proof and changes no credentials, runner, unit templates or activation states.
...
[legacy session status/rollback reference](../docs/proxmox-guarded-apply.md).
