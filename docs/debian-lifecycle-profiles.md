...
The guard runs before the apply guard and before the host lock. It only observes and refuses; it does not mount, enroll, restore, start, enable, or create protected state. The inactive unit set includes Docker socket activation and every declared Restic timer, target, worker, and recovery service. The inactive-path observer currently uses `lstat`, `ismount`, and path-based enumeration; it does **not** yet prove absence of same-filesystem bind mounts or prevent ancestor-symlink/racing path traversal. Descriptor-safe mount observation remains a completion blocker, not a qualified safety guarantee.
