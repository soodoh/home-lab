...
The independent recovery recipient is `age1ddk0qtwjclc2za5afrz5pl4j5kley02rqv2vh0s07c27a8t5u58sph58qm`. Its private identity remains controller-local under `~/.config/sops/home-lab-recovery`, mode `0600` in a mode `0700` directory, and a convenience copy is held in the home Vaultwarden service. On September 19 the operator confirmed an exact plaintext identity copy on an offline USB stored securely offsite. That USB is the canonical independent custody path; Vaultwarden is not, because its availability depends on the home lab.

Historical GPG ciphertext remains preserved but is a legacy optional envelope, not a required recovery dependency. Its separate decryptability is not asserted by the USB confirmation. A separately authorized September 19 [retrieval drill](../recovery/aws-bundle-retrieval.md) used the operator-accepted identical controller copy to verify publication-credential decryption and exact AWS/KMS historical bundle access. It did not literally read the USB, establish an independent repository checkout, decrypt the bundle or restore data; see the [custody decision](decisions.md#custody-is-separate-from-receipt-cleanup).
