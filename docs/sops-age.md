...
Historical GPG ciphertext remains preserved but is a legacy optional envelope, not a required recovery dependency. Its separate decryptability is not asserted by the USB confirmation. The confirmation closes age-key location custody only: independent repository access, publication-credential decryption, AWS/KMS bundle retrieval and restoration remain separately unqualified; see the [custody decision](decisions.md#custody-is-separate-from-receipt-cleanup) and [retrieval drill](../recovery/aws-bundle-retrieval.md).
