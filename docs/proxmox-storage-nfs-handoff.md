...
Provider and consumer parity are complete in check mode with `changed=0`: pool/dataset, export file/table, services, PVE registration, VM 100, Debian mount, Compose, and Restic all match. `zfs_dataset` is operationally transferred to Ansible after the Nix freeze, exact capability installation, source transfer, and committed no-mutation ownership receipt `fcfad9042abc3c11474b8deb41c2c5f96433f64a423b94d8667e883c62680689`. Its receipt is root-owned, mode `0600`, regular, single-link, and records `changed: false`; Nix has no representable dataset-property mutation action. `nfs_export_service` is also operationally transferred after its separate freeze, capability installation, source transfer, and committed no-mutation ownership receipt `4e9b1929d88ec40ac8c21e3840db53f1a8333b32ee6339d40f5aca4c06160743`. Its receipt has the same durable metadata and records exact export file/table and service parity with `changed: false`. No dataset property, topology, mount, export reload, service restart, VM, package, network, or reboot mutation occurred.
