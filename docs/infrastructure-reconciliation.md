...
The Proxmox Ansible-to-Nix migration has completed its contract/schema prerequisite only. Ordinary `managed-file`/`managed-directory` policy, shared `managed-file-metadata`, explicit `audit-absence` expectations, and remaining non-secret steady host semantics are centralized in the validated contract and consumed by temporary Ansible. Runtime-protected access records are explicitly metadata-only and non-projectable, while `/etc/pve` remains a non-projectable PVE API/CLI ownership domain. No Nix projector exists yet, and production continues to require the existing Ansible no-op gate until the separately reviewed shadow and cutover phases complete.
