...
The Proxmox Ansible-to-Nix migration now includes its controller-side foundation. The authoritative contract is projected through an explicit allowlist into tracked canonical, schema-closed JSON under `nix/proxmox/`; repository validation proves exact regeneration and exclusion of protected values, reference names, paths, controller/API locations, hardware identities, and migration/cleanup gates. The dedicated sanitized flake root is `nix/`, pinned to `nixos-26.05`; use `nix flake check 'path:./nix'` and `nix build 'path:./nix#proxmox-host-bundle'`. The explicit `path:` prefix prevents nested-flake Git resolution from archiving the repository root. No repository-root flake exists. Validation archives and scans the exact sanitized flake source, direct derivation source inputs, output, and closure. The flake builds a canonical content-hashed bundle containing the projection, current exact 1,353-package manifest with its count derived from hash-bound provenance, rendered safe ordinary files and audit expectations, bound metadata/protocol, and inert fixed observer/activator helpers generated solely from the reviewed bundle builder's deterministic template. The helpers have no external source-file or CLI input and expose only `version` and `self-check`, the flake exposes no plan/apply/verify apps, and Nix is not required on Proxmox. Production continues to use temporary Ansible and its no-op gate until the separately reviewed shadow and cutover phases complete.
