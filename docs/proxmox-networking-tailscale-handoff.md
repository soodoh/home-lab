...
Read-only provider and consumer parity is complete with `changed=0`. `network_interfaces` is operationally transferred to Ansible after the Nix freeze, exact capability installation, source transfer, and committed no-mutation ownership receipt `ab53a8144b9a5f102919f7698f033d14a64c151ecfbe0a5be6d8825338436a3f`. The root-owned mode-`0600`, regular, single-link receipt records `changed: false` and `runtime_activated: false`; no network file or runtime state changed. `tailscale_node` is also operationally transferred after its separate freeze, corrected capability installation, source transfer, and committed no-mutation ownership receipt `5919a587407f40bc3ab108365587a40925571543c736b66a7c5323be87e14dee`. Its durable receipt records `changed: false`, `state_mutated: false`, and `service_restarted: false`; no preference, protected state, service, package, policy, firewall, network, VM, or reboot mutation occurred. Resolver and hosts-file ownership remain excluded.
...
The Tailscale ownership capability is independently fixed and no-mutation. The host reduces status and preferences without exporting node, peer, endpoint, address, key, or state bytes; verifies the transferred network dependency, protected state-directory metadata, exact service/version/preferences, external-owner summaries, and lock absence; and can write only a mode-`0600` receipt recording `changed: false`, `state_mutated: false`, and `service_restarted: false`.
