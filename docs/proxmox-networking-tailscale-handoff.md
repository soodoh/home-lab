...
Read-only provider and consumer parity is complete with `changed=0`. `network_interfaces` is source-transferred to Ansible after the Nix freeze and exact capability installation; its separately authorized no-mutation ownership receipt remains required before operational completion. `tailscale_node` remains separately `pending` under Nix. Resolver and hosts-file ownership remain excluded.
