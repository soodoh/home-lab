...
Read-only provider and consumer parity is complete with `changed=0`. `network_interfaces` is operationally transferred to Ansible after the Nix freeze, exact capability installation, source transfer, and committed no-mutation ownership receipt `ab53a8144b9a5f102919f7698f033d14a64c151ecfbe0a5be6d8825338436a3f`. The root-owned mode-`0600`, regular, single-link receipt records `changed: false` and `runtime_activated: false`; no network file or runtime state changed. `tailscale_node` is separately source-transferred to Ansible after its Nix freeze and corrected exact capability installation; its separately authorized no-mutation ownership receipt remains required before operational completion. Resolver and hosts-file ownership remain excluded.
