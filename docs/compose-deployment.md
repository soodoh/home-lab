...
Every apply attempt retains the production lock on failure. Success requires an idempotent post-deployment Compose action plan, all 38 configured services running, healthy Gluetun and Seerr, a zero-change deployment post-check, and a zero-change complete audit. There is no automatic stateful rollback; the tracked previous artifact and unchanged previous environment are recovery inputs for a separately reviewed rollback. A failed pre-apply canary may be retried manually only with the exact candidate hash, typed confirmation for that exact candidate, the same canary-only policy, and `COMPOSE_AUTO_APPLY_ENABLED=true`.
