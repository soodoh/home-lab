...
The one-time Calibre local rollback is additionally bound to `rollback-calibre-to-local:<artifact-hash>`. It requires exactly the three Calibre consumers and the two Restic policy files, proves the live NFS mounts and exact NFS identity, stops all three writers, reconciles NFS into the retained local library with checksum/delete semantics, requires a zero-difference verification and SQLite `integrity_check`, and only then permits the normal dependency-aware Compose convergence to activate local mounts. Any pre-convergence failure retains the production lock and leaves the authoritative NFS source intact.
...
The Debian production endpoint is `docker-host` with dedicated login identity `ansible-deploy`; the `docker` workload account is not the deployment identity. Strict SSH host-key checking remains mandatory. `infrastructure/evidence/vm-100-debian-ssh-host-key.json` records the ED25519 key independently read through Proxmox QGA; retirement evidence binds that unchanged key to the final tailnet name before controllers connect.
