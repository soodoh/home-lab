...
Bounded existing-host Nix-runtime retirement completed 2026-09-11 with no removals or configuration changes under [ADR 0004](adr/0004-operational-nix-retirement.md) and the [completion checklist](host-lifecycle-completion-plan.md). This retained source/recovery reference records candidate history and **still-binding executable gates**, not a requirement to reopen retirement or implement predecessor/reporting/bootstrap mechanisms. It does not certify new-source convergence, universal Nix absence, fresh boot/backup/recovery success or a clean rebuild. ADR 0001's safety rules remain authoritative. This document is **not** installation, collection, qualification, package, reboot, recovery or deployment approval.

[ADR 0005](adr/0005-attended-operational-admission.md) explicitly defers this v6 participant transition. Its selected attended neutral observer/collector plus fixed `observe-admission` transport and enumerated read-only sudo policy are also deferred, local, undeployed and unqualified—not current maintenance defaults. Their source route retains shared `observe` for transaction snapshots and does not call this installer. ADR 0005 still binds every invocation; admission's erroneous rejection of the legitimate persistent firewall-recovery `operation.lock` pathname is not fixed. Preserve locks, journals and watchdogs; unknown pending operations and uncertain independent recovery access still bar relevant writes. Do not use ordinary `observe`, legacy private-preparer `summary`, v6, raw OpenTofu or direct ungated Ansible as bypasses. The complete accepted snapshot, including independent improvements and the attended implementation, is preserved on local, unpublished branch `wip/deferred-attended-controller-3491395-01`. Local `main` integrates the non-controller work and retains older v6 source, still deferred—not reactivated by parking the attended changes. This guide's v6 recovery/design reference does not make either branch operationally ready. Its VFIO/queued-reboot and predecessor gates remain unchanged. Attended evidence must not authorize this candidate or its deferred qualification consumers.
