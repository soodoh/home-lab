...
The contract now advances `boot_configuration` to `ready` while retaining `current_owner: nix`. The reduced Nix projection freezes automatic mutation for the GRUB fragment, ZFS module option, and VFIO modules-load file; the protected VFIO source remains nonprojectable. This is a freeze-only transition, not an ownership transfer, and requires physical-console installation and verification of the exact frozen bundle before `transferred` can be considered.
