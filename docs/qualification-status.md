...
- two-path encrypted local backup deployment across the games and NFS filesystems with matching newest-archive metadata
...
- guarded import, exact-volume reconciliation, and 64 GiB resize of the pinned Flatcar image on `scsi3`, with the Arch boot order retained, `scsi0`/`scsi1`/`scsi2` preserved, no unused disks, VM 100 still running, Arch audit `changed=0`/`failed=0`, all OpenTofu roots at zero actions, and Proxmox Nix at zero actions, recorded in `infrastructure/evidence/vm-100-flatcar-disk-import.json`
- guarded `ide2` cloud-init drive and `cicustom` attachment for the exact protected Ignition snippet, with activation explicitly pending the controlled reboot, Arch boot authority unchanged, VM 100 and its audit healthy, no unused disks, all OpenTofu roots at zero actions, and Proxmox Nix at zero actions, recorded in `infrastructure/evidence/vm-100-flatcar-ignition-attachment.json`
