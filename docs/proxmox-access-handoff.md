...
Proxmox package ownership is transferred to Ansible under no-mutation receipt `6b0f77137db3d19c84c16e5d38ca468723a85393c4f6209b46a190a026003055`. Nix observation uses the fixed `ansible-plan` Tailscale observer, protected evidence is complete, every production action policy is nonautomatic, and lifecycle policy now rejects both Nix `prepare` and `apply` before any `tofu-apply` transport. Local tofu identities still remain part of the protected-access summary and recovery inventory; remove that evidence dependency through a reviewed source transition before any account, key, sudo rule, helper, token, or LAN recovery retirement.
