...
Default-deny PVE firewall activation is not part of the ordinary steady play. It must follow the isolated pvesh-only, watchdog-backed, exact-plan design in [`proxmox-firewall-cutover.md`](proxmox-firewall-cutover.md). The design is not live execution approval.
