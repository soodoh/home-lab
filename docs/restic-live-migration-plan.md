...
The exact retention plan `6239f3c0a67c66d2a3b23ca7dfa84853391fca98bf8b5b9d116004925d6684ae` subsequently applied the safe-direction 365-day hold, and read-only verification confirmed lifecycle `365/1/1`, versioning enabled, delete-marker cleanup retained, and the same current object. A later unrelated Debian access convergence failed and was recovered forward without rolling back AWS. A separately reviewed IAM plan granted `s3:ListBucketVersions` to both controller roles and the protected recovery user; fresh controller sessions now produce the same exact current version-ID hash, but the protected recovery user has no active access key and the actual Offen backup principal remains denied. Keep the contract `planned` and Offen active. Through another exact IAM-only plan, add a bucket-policy allow for only `s3:ListBucketVersions` on the recovery bucket to `backup_principal_arn`; then use the existing protected backup credential pipeline to independently hash the exact current version ID without recording the raw ID before attesting the hold as `applied`.
