...
The operator subsequently authorized account-wide Proton Trash deletion. Saved plan `53898e93b53264768b090d4b00e7e3add22e822efa5c7fa1bb6fc0b97e0f8a90` invoked the exact `EmptyTrash` provider operation, but its required post-mutation v2 full-data canary failed, so no committed Trash-cleanup receipt exists. The durable host journal remains `cleanup-started`. Two independent `restic check --read-data` attempts failed, and a structural check now reports exactly broken pack `a1f72495c2ca8a84c76fb7843e2ebcd35f6b8d675bb8121a736c4c3622f1e669`. Daily and maintenance timers were stopped immediately. Do not rerun Trash cleanup, commit its receipt, restart scheduling, repair an index, forget/prune snapshots, or alter Proton v2 until a separately reviewed exact pack-repair/source-copy transaction restores a zero-error full-data check.

A separately reviewed replacement transaction then created the requested canonical active path `rclone:proton-backup:Backups/home-lab-restic` from the freshly full-read-verified games source under plan `6c128f8498c87fa070685f79608296855508b97818098a1300e77740a4ee58eb`. It reached repository ID `485946d1d9139ca58ac9110832203455aa474c1ca390ae53685f8ca7ca5e12ab` and copied snapshot `20c0447f1f8c730b08656018a3762fe4a82355c8f7d390b8a3af9eeca4a072ab`, but its immediate structural canary failed: pack `392b7b680b6a25aaab2779fd1a49b6b8be213bcfde46f9429262edb3741db63c` exists in Proton with size zero. The journal remains `copied`; no receipt was committed and the contract still targets v2. This reproduces the same zero-byte condition as v2's broken pack and demonstrates that the current Proton/rclone upload path is unsafe. Evidence is `infrastructure/evidence/proton-canonical-creation-failure.json`. Do not adopt, repair, retry, retire either active Proton directory, change the contract, or restart timers until the provider/client issue has a separately qualified remediation.
...
As of 2026-09-03, upstream still identifies v1.75.0 as the latest stable release, so no stable build containing the Proton fix is available. Production therefore remains on the exact qualified beta; a stable transition still requires a new hash-pinned qualification and recovery proof.
...
A return to TOTP uses canonical SOPS key `PROTON_BACKUP_TOTP_SECRET`; its value must never appear in chat, shell arguments, logs, evidence, or plaintext files. First run `prepare-proton-totp-transition.yml` to acquire and retain the production lock, stop both shared non-persistent Restic timers, and prove every Restic/rclone process inactive. Only then may the operator enable TOTP and add the canonical key directly with SOPS. After the candidate contract declares `authentication_mode: totp` and credential ref `proton_totp_secret`, `transition-proton-totp.yml` adopts only the exact retained owner, requires synchronized host time, decrypts SOPS only into helper stdin, and under the backup mutex atomically adds obscured `otp_secret_key` while removing either all four cached `client_*` fields or none. The resumable helper validates exact protected paths and static fields, makes zero provider requests, emits redacted evidence, and leaves both timers and the production lock retained.

Cached credentials cannot qualify TOTP. The separately reviewed live phase must force fresh authentication, verify the exact Proton repository identity and snapshot lineage, publish new TOTP-specific qualification evidence without relabeling historical password-only evidence, rebuild and independently prove recovery bundles A and B containing the protected TOTP secret, run one complete games-to-NFS-to-Proton chain, and pass a zero-change audit. Release the exact lock and restore the same daily and maintenance timers only after all checks pass. Failed authentication or recovery proof leaves timers stopped and the operation-specific lock retained; provider rollback requires a separate manual decision.

The guarded TOTP cutover completed on 2026-08-27. Fresh authentication opened exact Proton repository `d1faa9cd772dd13275b8d4db376c2bbba0b82a9415a28b0d03a8b17e37b7fb7e`; TOTP-specific qualification and recovery-bundle evidence are retained in `infrastructure/evidence/proton-totp-qualification.json` and `infrastructure/evidence/proton-totp-recovery-bundles.json`. Bundle A remains protected locally, and bundle B was published as a new checksum-verified KMS-encrypted version with the same independently encrypted plaintext. The terminal chained proof produced games snapshot `48c4701a2673a5c3e77dff4387de2c5c54c3eb6b1b940f3223db435641152d25`, NFS snapshot `49ff11996bce52054317c534dd83afb56af12e94493c8659f4a86c6581e2bc7a`, and Proton snapshot `694d8818b8ec6bc918134e0a116b2dfd820ae59ed0c3a1a7810253b1a3597dc6`. Both non-persistent timers are active and enabled; terminal evidence is `infrastructure/evidence/proton-totp-cutover.json`.

The dedicated account previously used password-only authentication after pinned rclone `1.75.0` authenticated successfully with TOTP but failed during Drive/key initialization on the original account state. That historical availability/security trade-off and its evidence remain retained. The guarded `transition-proton-password-only.yml` transaction removed only the then-obsolete `otp_secret_key` from the installed rclone config, required the client cache to be absent, preserved all other fields and prior evidence, made zero Proton requests, and retained the failed qualification lock:
