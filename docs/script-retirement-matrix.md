...
> Current sequencing is the [operational Nix retirement checklist](host-lifecycle-completion-plan.md). This is a historical per-file classification, not current caller counts or a deletion list; retain its recovery/retention cautions. The unused [Debian observer pair](../archive/lifecycle-experiments/debian-predecessor-observer/README.md) is archived separately under explicit owner approval. Other helpers/tests are unchanged.
...
| `scripts/controller/protected_execution.py` | `retain-controller` | 3 | `scripts/controller/lifecycle-marker-transaction.py`, `scripts/controller/proxmox-package-activation.py`, `scripts/controller/proxmox-reboot-activation.py` |
...
| `scripts/controller/proxmox-deploy-upgrade.py` | `retain-controller` | 1 | `scripts/controller/test-proxmox-access-transports.py` |
...
| `scripts/controller/proxmox-host-projection.js` | `retain-shared-projection` | 1 | `scripts/reconcile-infrastructure` |
