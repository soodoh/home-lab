...
- `production-activation` requires exact contract-derived mount sources/filesystems/UUIDs/options, the recovered recipient, the enrolled node, a digest-bound root environment, contract-selected Compose command/artifact/image override, a schema-validated Restic activation receipt and direct systemd dependencies. Every unit must initially be inactive. The executor records attempted starts in an in-memory list before invocation. Its exception handler attempts reverse-order stops and inactive-state checks. After an attempted marker publication, it restores the saved recovery-marker bytes only if the still-open publication descriptor and current pathname positively match in file identity, root-owned single-link regular `0600` metadata and exact published content. Same bytes on a foreign inode, changed bytes, missing/link/unsafe or otherwise ambiguous state are retained and reported as cleanup failure, with the original activation exception chained. This is caught-exception cleanup, not a crash-safe transaction or a guarantee that every prior state and downstream effect is restored.
...
### Production marker rollback (source only)

The publication descriptor remains open through caught-exception cleanup to pin
its inode against reuse. Publication intent is recorded before rename: an error
from rename or directory fsync is not proof that publication had no effect.
Proven-owned after-images can therefore be restored across that boundary. The
rollback before-image must be fully written and file-fsynced before a second
ownership check and rename; short writes never publish a truncated before-image.
Rollback creation/publication/verification failures remain failures even when
bounded unit cleanup completes. No automatic retry or startup recovery is added.
These checks assume trusted root-owned ancestry and cooperating writers under the
existing lock; pathname check then rename is not an atomic CAS against arbitrary
racing root writers. Abrupt death and durability after failed fsync remain unknown.

`python3 -I -B -S scripts/controller/test-debian-production-marker-rollback.py --green`
executes the actual host `production()` and its file/descriptor rollback logic
with stdlib-only, closed filesystem, metadata, clock and command adapters. It
retains fresh private controller fixtures and source/log evidence under `/tmp`.
`--red` selects the single foreign-bytes protection test; `--mutant` bypasses the
actual ownership check in a retained source copy and must fail that test. Fixtures
contain only synthetic inputs, with root metadata modeled explicitly. Modeled
starts and empty mount parameters are premises for reaching marker cleanup, not
proof of controller admission, the retained guard admitting activation, native
systemd behavior, installed bytes, boot or recovery. No real guard, gate, token,
credential, backup receipt or production data is used. The wider migration and
failed VM9900 qualification remain unchanged and require separate decisions.
