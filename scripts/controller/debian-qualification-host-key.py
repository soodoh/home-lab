...
import argparse,datetime as dt,hashlib,importlib.util,ipaddress,json,os,re,subprocess
...
ROOT=Path(__file__).resolve().parents[2]; SOURCE=ROOT/"scripts/controller/debian-qualification-snippet.py"; spec=importlib.util.spec_from_file_location("qualification_snippet",SOURCE); snippet=importlib.util.module_from_spec(spec); spec.loader.exec_module(snippet); CLEAN_SOURCE=ROOT/"scripts/controller/debian-qualification-first-boot.py"; clean_spec=importlib.util.spec_from_file_location("qualification_first_boot",CLEAN_SOURCE); clean=importlib.util.module_from_spec(clean_spec); clean_spec.loader.exec_module(clean)
...
def clean_boot(path,admission_path,known_hosts,target,stopped_receipt):
 ...
 command=["node",str(clean.ADMISSION),"--evidence",str(admission_path),"--known-hosts",str(known_hosts)]; checked=subprocess.run(command,text=True,capture_output=True)
 ...
 if set(value)!=required or value.get("format")!="home-lab-debian-qualification-clean-first-boot-receipt-v1" or value.get("status")!="verified" or value.get("target_id")!=target["target_id"] or value.get("vmid")!=9900 or value.get("commit")!=revision() or value.get("admission_sha256")!=sha(admission_raw) or prior_target.get("admission_mode") not in ("fresh","expired-safe-stop") or any(prior_target.get(key)!=target.get(key) for key in fields) or value.get("start_receipt_sha256")!=stopped_receipt["prior_receipt_sha256"] or value.get("observation_sha256")!=sha(canonical_bytes(observation)+b"\n") or any(value.get(key)!=item for key,item in producer.items()) or observed>now()+dt.timedelta(seconds=5) or observed<now()-dt.timedelta(hours=4): fail("clean-boot-receipt")
...
def plan(args):
 ...
 try:
  target=snippet.validate_target(args.admission,args.known_hosts); state=output/"state.tfstate"; receipt,receipt_sha=stopped(args.stopped_receipt,target,state); guest_ipv4,clean_sha=clean_boot(args.clean_boot_receipt,args.clean_boot_admission,args.known_hosts,target,receipt); created=now()
  value={"admission_sha256":target["isolation_attestation_sha256"],"authorized":False,"automatic_apply":False,"clean_boot_receipt_sha256":clean_sha,"commit":revision(),"created_at":created.isoformat().replace("+00:00","Z"),"disk_volume":"local-lvm:vm-9900-disk-0","expires_at":(created+dt.timedelta(hours=4)).isoformat().replace("+00:00","Z"),"format":"home-lab-debian-qualification-host-key-plan-v1","guest_ipv4":guest_ipv4,"node_name":target["node_name"],"operation":"extract-host-key-read-only","state_sha256":receipt["state_sha256"],"stopped_receipt_sha256":receipt_sha,"target_id":target["target_id"],"version":1,"vmid":9900}
...
def apply(args):
 ...
 target=snippet.validate_target(args.admission,args.known_hosts); state=output/"state.tfstate"; stopped_receipt,receipt_sha=stopped(args.stopped_receipt,target,state); guest_ipv4,clean_sha=clean_boot(args.clean_boot_receipt,args.clean_boot_admission,args.known_hosts,target,stopped_receipt); revision(value.get("commit"))
 required={"admission_sha256","authorized","automatic_apply","clean_boot_receipt_sha256","commit","created_at","disk_volume","expires_at","format","guest_ipv4","node_name","operation","state_sha256","stopped_receipt_sha256","target_id","version","vmid"}; created=parse_time(value.get("created_at","")); expires=parse_time(value.get("expires_at",""))
 if set(value)!=required or value.get("format")!="home-lab-debian-qualification-host-key-plan-v1" or value.get("operation")!="extract-host-key-read-only" or value.get("target_id")!=target["target_id"] or value.get("node_name")!=target["node_name"] or value.get("disk_volume")!="local-lvm:vm-9900-disk-0" or value.get("guest_ipv4")!=guest_ipv4 or value.get("clean_boot_receipt_sha256")!=clean_sha or value.get("state_sha256")!=stopped_receipt["state_sha256"] or value.get("stopped_receipt_sha256")!=receipt_sha or value.get("version")!=1 or value.get("vmid")!=9900 or value.get("authorized") is not False or value.get("automatic_apply") is not False or created>now()+dt.timedelta(seconds=5) or created<now()-dt.timedelta(hours=4) or expires<=now() or expires-created>dt.timedelta(hours=4): fail("plan-binding")
 ...
 expected={"clean_boot_receipt_sha256":clean_sha,"disk_volume":value["disk_volume"],"format":"home-lab-debian-qualification-host-key-receipt-v1","guest_ipv4":value["guest_ipv4"],"plan_sha256":args.plan_sha,"state_sha256":value["state_sha256"],"stopped_receipt_sha256":receipt_sha,"target_id":value["target_id"],"version":1,"vmid":9900}
...
def main():
 ...
 for name in ("plan","apply"):
  item=sub.add_parser(name); item.add_argument("--admission",type=Path,required=True); item.add_argument("--known-hosts",type=Path,required=True); item.add_argument("--stopped-receipt",type=Path,required=True); item.add_argument("--clean-boot-receipt",type=Path,required=True); item.add_argument("--clean-boot-admission",type=Path,required=True); item.add_argument("--output-dir",type=Path,required=True)
