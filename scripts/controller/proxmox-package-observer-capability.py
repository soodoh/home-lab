...
def observe():
 program='''import hashlib,json,os,stat,subprocess\npaths={}\nfor p in %r:\n try:\n  before=os.lstat(p)\n  if not stat.S_ISREG(before.st_mode) or before.st_nlink!=1 or before.st_size>1048576: paths[p]={"exists":True,"uid":before.st_uid,"gid":before.st_gid,"mode":format(stat.S_IMODE(before.st_mode),"04o"),"nlink":before.st_nlink,"regular":False}; continue\n  fd=os.open(p,os.O_RDONLY|os.O_NOFOLLOW); after=os.fstat(fd); raw=os.read(fd,1048577); os.close(fd)\n  if (before.st_dev,before.st_ino,before.st_size)!=(after.st_dev,after.st_ino,after.st_size) or len(raw)>1048576: raise RuntimeError("artifact changed during observation")\n  paths[p]={"exists":True,"uid":after.st_uid,"gid":after.st_gid,"mode":format(stat.S_IMODE(after.st_mode),"04o"),"nlink":after.st_nlink,"regular":True,"sha256":hashlib.sha256(raw).hexdigest()}\n except FileNotFoundError: paths[p]={"exists":False}\nlocks=[p for p in ("/var/lib/iac-ansible-production.lock","/var/lib/home-lab/reconciliation/owner.lock","/var/lib/home-lab/reconciliation/nix.lock","/var/lib/home-lab/firewall-transaction/active.json") if os.path.lexists(p)]\nmutexes=("/var/lib/home-lab/reconciliation/operation.lock","/run/lock/home-lab-proxmox-package.lock","/run/lock/home-lab-proxmox-reboot.lock","/run/lock/home-lab-vfio-recovery.lock")\nfor p in mutexes:\n if os.path.lexists(p):\n  s=os.lstat(p)\n  if not stat.S_ISREG(s.st_mode) or s.st_uid!=0 or s.st_gid!=0 or s.st_nlink!=1 or stat.S_IMODE(s.st_mode)!=0o600: locks.append(p+":unsafe-metadata")\nheld=subprocess.run(("/usr/bin/lslocks","--json","--output","PATH"),capture_output=True,text=True,check=True)\nheld_paths={item.get("path") for item in json.loads(held.stdout).get("lockspaces",[]) if isinstance(item,dict)}\nlocks.extend(p for p in mutexes if p in held_paths)\nprint(json.dumps({"paths":paths,"locks":sorted(locks)},sort_keys=True,separators=(",",":")))\n''' % ((TRANSPORT,OBSERVER,SUDO),)
