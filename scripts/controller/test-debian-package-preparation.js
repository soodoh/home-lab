...
assert.match(script, /systemctl disable --now "\$\{DOCKER_SERVICES\[@\]\}"/);
assert.match(script, /systemctl mask "\$\{SERVICES\[@\]\}"/);
assert.match(script, /systemctl unmask "\$\{DOCKER_SERVICES\[@\]\}"/);
assert.match(script, /enabled=\$\(systemctl is-enabled "\$RUNTIME_SERVICE"/);
assert.match(script, /\[\[ \$enabled == masked \]\]/);
