...
invalidDebianPackageScope.lifecycle.maintenance.package_plan.hosts.debian.allowed_apply_scope = "security-updates-only";
check(invalidDebianPackageScope, false, "Debian package apply requires an exact reviewed set");
const invalidAutomaticPackageApply = structuredClone(contract);
invalidAutomaticPackageApply.lifecycle.maintenance.package_plan.hosts.debian.automatic_apply = true;
check(invalidAutomaticPackageApply, false, "package mutation cannot be automatic");
...
const invalidTransferredTailscaleOwner = structuredClone(contract);
invalidTransferredTailscaleOwner.lifecycle.hosts.proxmox.domain_handoffs.tailscale_node.current_owner = "nix";
check(invalidTransferredTailscaleOwner, false, "transferred Tailscale node handoff requires Ansible ownership");
...
const missingRootKeyAttribution = structuredClone(contract);
delete missingRootKeyAttribution.lifecycle.hosts.proxmox.access_cutover.root_key_attributions["SHA256:/qSECkXxkpCIjTkBwa8XZZdRW2/seScon5uAKGlLC80"];
check(missingRootKeyAttribution, false, "every retained root key requires exact attribution");
const invalidRetiredRootGroup = structuredClone(contract);
invalidRetiredRootGroup.lifecycle.hosts.proxmox.access_cutover.retire_root_supplementary_groups = [];
check(invalidRetiredRootGroup, false, "stale apex membership remains an explicit retirement target");
