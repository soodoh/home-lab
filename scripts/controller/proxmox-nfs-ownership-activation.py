#!/usr/bin/env python3
"""Build and apply one no-mutation Proxmox NFS export/service ownership receipt."""
...
OUTPUT = ROOT / ".local/proxmox-nfs-ownership-activations"
LOCK = ROOT / ".local/locks/proxmox-nfs-ownership.lock"
...
def clean_pushed_commit() -> str:
    ...
    if commit != git("rev-parse", "origin/main") or git("status", "--porcelain=v1", "--untracked-files=all"):
        raise SystemExit("NFS ownership activation requires clean pushed HEAD")
    return commit


def transferred_contract() -> None:
    script = "const fs=require('node:fs'),{load}=require('js-yaml');const v=load(fs.readFileSync(process.argv[1],'utf8'));process.stdout.write(JSON.stringify(v.lifecycle.hosts.proxmox.domain_handoffs.nfs_export_service));"
    ...
    if result.returncode or result.stderr or json.loads(result.stdout) != expected:
        raise SystemExit("NFS export/service ownership is not transferred to Ansible")


def observe() -> dict:
    result = subprocess.run((*SSH, "observe nfs-lifecycle"), capture_output=True, timeout=120)
    if result.returncode or result.stderr:
        raise SystemExit("NFS ownership observation failed")
    value = json.loads(result.stdout)
    if value.get("format") != "home-lab-proxmox-nfs-ownership-observation-v1" or \
        ...
        raise SystemExit("NFS ownership parity is incomplete")
    return value
...
def build() -> tuple[Path, str]:
    ...
    value = {
        ...
        "format": "home-lab-proxmox-nfs-ownership-activation-v1", "host_key_fingerprint": FINGERPRINT,
...
def load(path: Path) -> tuple[dict, bytes, str]:
    ...
    if not stat.S_ISREG(metadata.st_mode) or stat.S_ISLNK(metadata.st_mode) or metadata.st_uid != os.getuid() or \
        ...
        raise SystemExit("NFS ownership activation metadata differs")
...
def controller_lock() -> int:
    ...
    try:
        ...
        os.close(descriptor)
        raise SystemExit("NFS ownership controller lock is active") from error
    return descriptor


def apply(path: Path) -> None:
    value, raw, digest = load(path)
    expected = f"apply-proxmox-nfs-ownership-{digest}"
    if os.environ.get("PROXMOX_NFS_OWNERSHIP_APPLY_CONFIRMED") != expected:
    ...
    if any(value.get(key) != expected_value for key, expected_value in bindings.items()) or \
        ...
        raise SystemExit("NFS ownership activation binding or freshness differs")
    transferred_contract()
    consumer_parity()
    if observe() != value.get("before"):
        raise SystemExit("NFS ownership observation changed after planning")
    descriptor = controller_lock()
    try:
        staged = subprocess.run((*SSH, f"stage nfs-ownership {digest}"), input=raw, capture_output=True, timeout=120)
        if staged.returncode or staged.stderr or json.loads(staged.stdout) != {"staged": True}:
            raise SystemExit("NFS ownership staging failed")
        result = subprocess.run((*SSH, f"apply nfs-ownership {digest}"), capture_output=True, timeout=300)
    finally:
        os.close(descriptor)
    if result.returncode or result.stderr:
        raise SystemExit("NFS ownership activation failed")
    output = json.loads(result.stdout)
    if output.get("nfs_ownership_transaction") not in {"committed", "already-committed"} or \
            output.get("changed") is not False or output.get("plan_sha256") != digest:
        raise SystemExit("NFS ownership activation result differs")
