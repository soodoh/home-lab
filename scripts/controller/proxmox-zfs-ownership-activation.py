#!/usr/bin/env python3
"""Build and apply one no-mutation Proxmox ZFS dataset ownership receipt."""
...
OUTPUT = ROOT / ".local/proxmox-zfs-ownership-activations"
LOCK = ROOT / ".local/locks/proxmox-zfs-ownership.lock"
...
ROLE = ROOT / "ansible/roles/proxmox_storage_lifecycle/tasks/main.yml"
CONSUMER_ROLE = ROOT / "ansible/roles/proxmox_storage_consumers/tasks/main.yml"
...
def clean_pushed_commit() -> str:
    ...
    if commit != git("rev-parse", "origin/main") or git("status", "--porcelain=v1", "--untracked-files=all"):
        raise SystemExit("ZFS ownership activation requires clean pushed HEAD")
    return commit


def transferred_contract() -> None:
    script = "const fs=require('node:fs'),{load}=require('js-yaml');const v=load(fs.readFileSync(process.argv[1],'utf8'));process.stdout.write(JSON.stringify(v.lifecycle.hosts.proxmox.domain_handoffs.zfs_dataset));"
    ...
    if result.returncode or result.stderr or json.loads(result.stdout) != expected:
        raise SystemExit("ZFS dataset ownership is not transferred to Ansible")


def observe() -> dict:
    result = subprocess.run((*SSH, "observe storage-lifecycle"), capture_output=True, timeout=120)
    if result.returncode or result.stderr:
        raise SystemExit("ZFS ownership observation failed")
    value = json.loads(result.stdout)
    if value.get("format") != "home-lab-proxmox-zfs-ownership-observation-v1" or \
            value.get("parity_complete") is not True or value.get("protected_values_exported") is not False or \
            value.get("member_identities_exported") is not False or value.get("active_lifecycle_locks"):
        raise SystemExit("ZFS ownership parity is incomplete")
    return value


def consumer_parity() -> None:
    result = subprocess.run(("ansible-playbook", "-i", "inventory/production.yml", "playbooks/proxmox-storage-plan.yml",
...
def build() -> tuple[Path, str]:
    ...
    value = {
        ...
        "format": "home-lab-proxmox-zfs-ownership-activation-v1", "host_key_fingerprint": FINGERPRINT,
...
def load(path: Path) -> tuple[dict, bytes, str]:
    ...
    if not stat.S_ISREG(metadata.st_mode) or stat.S_ISLNK(metadata.st_mode) or metadata.st_uid != os.getuid() or \
        ...
        raise SystemExit("ZFS ownership activation metadata differs")
...
def controller_lock() -> int:
    ...
    try:
        ...
        os.close(descriptor)
        raise SystemExit("ZFS ownership controller lock is active") from error
    return descriptor


def apply(path: Path) -> None:
    value, raw, digest = load(path)
    expected = f"apply-proxmox-zfs-ownership-{digest}"
    if os.environ.get("PROXMOX_ZFS_OWNERSHIP_APPLY_CONFIRMED") != expected:
    ...
    if any(value.get(key) != expected_value for key, expected_value in bindings.items()) or \
        ...
        raise SystemExit("ZFS ownership activation binding or freshness differs")
    transferred_contract()
    consumer_parity()
    if observe() != value.get("before"):
        raise SystemExit("ZFS ownership observation changed after planning")
    descriptor = controller_lock()
    try:
        staged = subprocess.run((*SSH, f"stage storage-ownership {digest}"), input=raw, capture_output=True, timeout=120)
        if staged.returncode or staged.stderr or json.loads(staged.stdout) != {"staged": True}:
            raise SystemExit("ZFS ownership staging failed")
        result = subprocess.run((*SSH, f"apply storage-ownership {digest}"), capture_output=True, timeout=300)
    finally:
        os.close(descriptor)
    if result.returncode or result.stderr:
        raise SystemExit("ZFS ownership activation failed")
    output = json.loads(result.stdout)
    if output.get("zfs_ownership_transaction") not in {"committed", "already-committed"} or \
            output.get("changed") is not False or output.get("plan_sha256") != digest:
        raise SystemExit("ZFS ownership activation result differs")
