#!/usr/bin/env python3
"""Build and apply one no-mutation Proxmox network-interface ownership receipt."""
...
OUTPUT = ROOT / ".local/proxmox-network-ownership-activations"
LOCK = ROOT / ".local/locks/proxmox-network-ownership.lock"
...
def transferred_contract() -> None:
    script = "const fs=require('node:fs'),{load}=require('js-yaml');const v=load(fs.readFileSync(process.argv[1],'utf8'));process.stdout.write(JSON.stringify(v.lifecycle.hosts.proxmox.domain_handoffs.network_interfaces));"
    ...
    if result.returncode or result.stderr or json.loads(result.stdout) != expected:
        raise SystemExit("network interface ownership is not transferred to Ansible")


def observe() -> dict:
    result = subprocess.run((*SSH, "observe network-lifecycle"), capture_output=True, timeout=120)
    if result.returncode or result.stderr:
        raise SystemExit("NETWORK ownership observation failed")
    value = json.loads(result.stdout)
    if value.get("format") != "home-lab-proxmox-network-ownership-observation-v1" or \
...
def build() -> tuple[Path, str]:
    ...
    value = {
        ...
        "format": "home-lab-proxmox-network-ownership-activation-v1", "host_key_fingerprint": FINGERPRINT,
...
def apply(path: Path) -> None:
    ...
    expected = f"apply-proxmox-network-ownership-{digest}"
    if os.environ.get("PROXMOX_NETWORK_OWNERSHIP_APPLY_CONFIRMED") != expected:
    ...
    try:
        staged = subprocess.run((*SSH, f"stage network-ownership {digest}"), input=raw, capture_output=True, timeout=120)
        ...
        result = subprocess.run((*SSH, f"apply network-ownership {digest}"), capture_output=True, timeout=300)
    finally:
        os.close(descriptor)
    if result.returncode or result.stderr:
        raise SystemExit("NETWORK ownership activation failed")
    output = json.loads(result.stdout)
    if output.get("network_ownership_transaction") not in {"committed", "already-committed"} or \
