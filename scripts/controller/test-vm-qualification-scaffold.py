...
class VmQualificationScaffoldTests(unittest.TestCase):
    ...
    def test_root_is_inactive_and_state_is_isolated(self) -> None:
        ...
        self.assertIn('.qcow2"', self.main)
        self.assertIn('import_from  = proxmox_download_file.arch_cloud_image[0].id', self.main)
        self.assertNotIn('file_id      = proxmox_download_file.arch_cloud_image[0].id', self.main)
        self.assertNotIn('ssh {', self.main)
