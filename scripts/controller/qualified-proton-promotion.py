...
def create_plan() -> None:
    commit=C.clean_pushed_commit(); contract,raw=contract_policy(); observed=observe_host(); validate_observation(observed,contract); created=datetime.now(timezone.utc).replace(microsecond=0)
