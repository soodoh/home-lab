...
const schema = JSON.parse(fs.readFileSync(path.join(root, "infrastructure/host-lifecycle/proxmox/projection.schema.json"), "utf8"));
...
if (vfioScriptFile.content !== fs.readFileSync(path.join(root, "infrastructure/host-lifecycle/proxmox/vfio-recover.py"), "utf8")) {
...
if (JSON.stringify(storageHandoffs.nfs_export_service) !== JSON.stringify(transferredStorageHandoff)) {
  throw new Error("transferred NFS handoff differs");
}
...
const transferredPackageHandoff = { current_owner: "ansible", target_owner: "ansible", state: "transferred", parity_required: true, single_writer: true };
