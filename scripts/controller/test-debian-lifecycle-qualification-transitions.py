...
with tempfile.TemporaryDirectory(dir=ROOT/".local") as directory:
 ...
 try: module.prior(args,"stop",{**fresh,"admission_mode":"expired-safe-stop"}); raise AssertionError("expired cross-admission stop accepted")
 except SystemExit as error: assert "stop-lineage-requires-fresh-admission" in str(error)
 restarted={**direct,"format":"home-lab-debian-qualification-restart-receipt-v1","guest_ipv4":"192.168.0.53","guest_ipv4_observation_sha256":"4"*64,"host_key_receipt_sha256":"3"*64,"operation":"restart"}; path.write_text(json.dumps(restarted,sort_keys=True,separators=(",",":"))+"\n"); assert module.prior(args,"stop",admitted)[0]==restarted
 legacy={key:item for key,item in restarted.items() if key not in ("guest_ipv4","guest_ipv4_observation_sha256")}; legacy["commit"]="d29a47cff1677ac5eb1d46d9e6d3a2a4d9f77bb4"; path.write_text(json.dumps(legacy,sort_keys=True,separators=(",",":"))+"\n"); assert module.prior(args,"stop",admitted)[0]==legacy
 forged=dict(legacy); forged["commit"]="b"*40; path.write_text(json.dumps(forged,sort_keys=True,separators=(",",":"))+"\n")
 try: module.prior(args,"stop",admitted); raise AssertionError("unbound legacy restart receipt accepted")
 except SystemExit as error: assert "prior-receipt" in str(error)
 path.write_text(json.dumps(direct,sort_keys=True,separators=(",",":"))+"\n")
 try: module.prior(args,"stop",{"admission_mode":"fresh","isolation_attestation_sha256":"2"*64,"target_id":"other-target"})
 except SystemExit as error: assert "prior-receipt" in str(error)
 else: raise AssertionError("prior receipt target substitution accepted")
with tempfile.TemporaryDirectory(dir=ROOT/".local") as directory:
 host_receipt={"clean_boot_receipt_sha256":"e"*64,"disk_volume":"local-lvm:vm-9900-disk-0","fingerprint":"SHA256:"+"A"*43,"format":"home-lab-debian-qualification-host-key-receipt-v1","guest_ipv4":"192.168.0.53","plan_sha256":"a"*64,"public_key":"ssh-ed25519 "+"A"*44,"sha256":"b"*64,"state_sha256":"c"*64,"stopped_receipt_sha256":"d"*64,"target_id":"production-pve-vm9900-qualification","version":1,"vmid":9900}; host_receipt["sha256"]=module.sha((host_receipt["public_key"]+"\n").encode()); path=Path(directory)/"host-key.json"; path.write_text(json.dumps(host_receipt,sort_keys=True,separators=(",",":"))+"\n"); path.chmod(0o600); args=SimpleNamespace(host_key_receipt=path); admitted={"target_id":"production-pve-vm9900-qualification"}; assert module.host_key(args,"restart",admitted,"d"*64,"c"*64)==(module.sha(path.read_bytes()),"192.168.0.53")
