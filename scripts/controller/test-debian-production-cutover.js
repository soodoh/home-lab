...
assert.match(failedReconcile, /\.phase == "failed" or \.phase == "committed"/);
...
assert.match(coordinator, /rejection:"post-reboot-health"[\s\S]*mv -T "\$temporary" "\$failed"[\s\S]*rm -f -- "\$output"/);
assert.match(failedReconcile, /rejected production marker[\s\S]*rejection == "post-reboot-health"[\s\S]*rm -f -- "\$OUTPUT_MARKER"/);
assert.match(failedReconcile, /activation_basis=.*del\(\.phase,\.failedAt,\.rejection\)[\s\S]*failed_basis=.*del\(\.phase,\.failedAt,\.rejection\)[\s\S]*\$activation_basis == "\$failed_basis"/);
...
assert.ok(coordinator.includes("failed_reconcile_sha=57e8ce252053704d3f0f401abfae86054d260eb5945091a732d2b1363854f3e0"));
