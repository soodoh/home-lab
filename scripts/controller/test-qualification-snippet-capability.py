...
role=(ROOT/"ansible/roles/qualification_snippet_capability/tasks/main.yml").read_text(); sudo=(ROOT/"infrastructure/qualification/host/qualification-apply.sudoers").read_text(); capability=role+sudo; inventory=(ROOT/"ansible/inventory/proxmox-qualification-bootstrap.yml").read_text(); playbook=(ROOT/"ansible/playbooks/install-qualification-snippet-capability.yml").read_text(); site=(ROOT/"ansible/playbooks/site.yml").read_text()
for required in ("lifecycle_profile == 'maintenance'","ansible_user == 'proxmox'","qualification_snippet_capability_contract.lifecycle.qualification_route.mode","production_vm_mutation_allowed","production_disk_attachment_allowed","production_state_allowed","temporary_tailnet_user","conventional_ssh_key_allowed","production-pve-vm9900-qualification","disposable_pve_admission_sha256","install-production-pve-vm9900-qualification-capability","/storage/local","backup,import,iso,snippets,vztmpl","/var/lib/vz/snippets","uid: 1900","gid: 1900","password_lock: true","debian-qualification-snippet-transport","debian-qualification-snippet-transaction","state: absent", "visudo --check --file=%s","hold-lock *","host-key *","diagnostic *","qualification-diagnostic-attempts","offline_diagnostic.attempt_journal_directory","historical_stopped_receipt_lineage_with_fresh_admission","first-boot","mutation_authorized: false"):
