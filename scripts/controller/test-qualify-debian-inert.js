...
assert.doesNotMatch(script, /validate-contract|(?:^|\n)\s*node\s+/);
...
assert.match(transitionLibrary, /timeout 15 qm guest cmd "\$VMID" shutdown/);
assert.match(transitionLibrary, /ARCH_RESTORE_REQUIRES_HOST_REBOOT/);
assert.match(transitionLibrary, /Exact Arch boot path prepared; a physical Proxmox host reboot is required/);
...
assert.match(script, /\(\$prepare_mode == true \|\| \$rehearsal_mode == true \|\| \$identity_mode == true \|\| \$credentials_mode == true \|\| \$canary_mode == true \|\| \$production_mode == true\) && \$transition_started == true[\s\S]*prepare_arch_host_reboot_path/);
