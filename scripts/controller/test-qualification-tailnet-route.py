...
assert 'users  = ["proxmox", "ansible-deploy", "firewall-apply", "qualification-apply"]' in policy
assert 'users  = ["ansible-deploy", "firewall-apply", "qualification-apply"]' in policy
assert 'accept = ["proxmox", "ansible-deploy", "firewall-apply", "qualification-apply"]' in policy
assert 'deny   = ["ansible-plan", "docker", "root", "tofu-plan", "tofu-apply"]' in policy
