...
const flatcarScript = fs.readFileSync(path.join(root, "scripts/boot-flatcar-first"), "utf8");
const transitionLibrary = fs.readFileSync(path.join(root, "scripts/lib/vm-100-os-transition"), "utf8");
const script = `${flatcarScript}\n${transitionLibrary}`;
...
assert.equal(contract.flatcar.qualification_stage, "hardware-blocked");
assert.match(flatcarScript, /source scripts\/lib\/vm-100-os-transition/);
...
assert.match(script, /exec_without_transition_lock_fds qm start "\$VMID"/);
assert.match(script, /exec_without_transition_lock_fds \/usr\/local\/sbin\/home-lab-vfio-recover/);
assert.match(script, /exec 8>&-/);
assert.match(script, /exec 9>&-/);
...
assert.match(script, /timeout 3 bash -c "exec 3<>\/dev\/tcp\/\$FLATCAR_IP\/22"/);
