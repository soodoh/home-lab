...
function main(){
 ...
 if((!expiredMode&&!fresh)||(allowExpiredSafeStop&&!allowExpiredOfflineDiagnostic&&!boundedExpired)||(allowExpiredOfflineDiagnostic&&!fresh&&!boundedExpired)) fail("admission evidence is stale or has an unsafe validity window");
 ...
 if(route.mode!=="production-pve-disposable-vm"||route.vmid!==9900||route.production_vm_mutation_allowed!==false||route.production_disk_attachment_allowed!==false||route.production_state_allowed!==false||route.reuse_production_api_plan_apply_identities!==true||route.historical_stopped_receipt_lineage_with_fresh_admission!==true||route.cross_admission_safe_stop_with_fresh_admission!==true) fail("contract qualification route is unsafe");
 if(allowExpiredOfflineDiagnostic&&(route.offline_diagnostic.read_only!==true||route.offline_diagnostic.requires_stopped_receipt!==true||route.offline_diagnostic.disk_volume!=="local-lvm:vm-9900-disk-0"||route.offline_diagnostic.admission_expiry_grace_seconds!==14400||route.offline_diagnostic.attempt_journal_directory!=="/var/lib/home-lab/reconciliation/qualification-diagnostic-attempts"||route.offline_diagnostic.historical_stopped_receipt_lineage_with_fresh_admission!==true)) fail("contract offline diagnostic policy is unsafe");
 ...
 const admissionMode=fresh?"fresh":allowExpiredSafeStop?"expired-safe-stop":"expired-offline-diagnostic";
