...
function main(){
 ...
 process.stdout.write(JSON.stringify({admitted:true,bridge:evidence.network.bridge,controller_ipv4:evidence.network.controller_ipv4,disk_datastore_id:evidence.storage.disk_datastore_id,endpoint:evidence.endpoint,guest_ssh_public_key_sha256:evidence.credentials.guest_ssh_public_key_sha256,image_datastore_id:evidence.storage.image_datastore_id,isolation_attestation_sha256:sha(raw),node_name:evidence.node_name,ssh_address:evidence.host_key.ssh_address,ssh_agent_public_key_sha256:evidence.credentials.ssh_agent_public_key_sha256,ssh_username:sshPrincipal,target_id:evidence.target_id})+"\n");
}
