...
def verify_agent(expected,public_key_path):
 ...
 try: public=raw.decode("ascii").removesuffix("\n")
 except UnicodeDecodeError as error: raise SystemExit("PVE qualification public key is not ASCII") from error
 fields=public.split()
 if raw!=public.encode()+b"\n" or len(fields)!=3 or fields[0]!="ssh-ed25519" or not fields[2].startswith("qualification-") or sha(f"{fields[0]} {fields[1]}".encode())!=expected: raise SystemExit("PVE qualification public key identity mismatch")
 result=subprocess.run(["ssh-add","-L"],text=True,capture_output=True); lines=[line for line in result.stdout.splitlines() if line.strip()]
 if result.returncode or len(lines)!=1: raise SystemExit("qualification SSH agent must contain exactly one key")
 offered=lines[0].split()
 if len(offered)<2 or f"{offered[0]} {offered[1]}"!=f"{fields[0]} {fields[1]}": raise SystemExit("qualification SSH agent identity mismatch")
def ssh_args(target,known_hosts,pve_public_key,command):
 return ["ssh","-F","/dev/null","-T","-o","BatchMode=yes","-o","StrictHostKeyChecking=yes","-o","GlobalKnownHostsFile=/dev/null","-o","UpdateHostKeys=no","-o",f"UserKnownHostsFile={known_hosts}","-o","IdentitiesOnly=yes","-o",f"IdentityFile={pve_public_key}","-o","PreferredAuthentications=publickey","-o","PasswordAuthentication=no","-o","KbdInteractiveAuthentication=no","-o","ClearAllForwardings=yes","-o","PermitLocalCommand=no","-o","RequestTTY=no","-o","ConnectTimeout=10",f'{target["ssh_username"]}@{target["ssh_address"]}',command]
def remote(target,known_hosts,pve_public_key,command,data=None):
 result=subprocess.run(ssh_args(target,known_hosts,pve_public_key,command),input=data,capture_output=True,timeout=90)
...
def plan(args):
 target=validate_target(args.admission,args.known_hosts); verify_agent(target["ssh_agent_public_key_sha256"],args.pve_public_key); public=guest_key(args.guest_public_key,target); commit=clean_commit(); before=remote(target,args.known_hosts,args.pve_public_key,"observe")
...
def apply(args):
 ...
 target=validate_target(args.admission,args.known_hosts); verify_agent(target["ssh_agent_public_key_sha256"],args.pve_public_key); public=guest_key(args.guest_public_key,target); verify_exact_checkout("git",value.get("commit",""),os.environ.copy())
 ...
 current=remote(target,args.known_hosts,args.pve_public_key,"observe")
 ...
 try: receipt=remote(target,args.known_hosts,args.pve_public_key,f"apply {digest} {digest}",raw)
...
def verify(args):
 target=validate_target(args.admission,args.known_hosts); verify_agent(target["ssh_agent_public_key_sha256"],args.pve_public_key); public=guest_key(args.guest_public_key,target); receipt,_=load_canonical_object(args.receipt,"snippet receipt"); content=render(public); current=remote(target,args.known_hosts,args.pve_public_key,"observe")
...
def main():
 ...
 for name in ("plan","apply","verify"):
  item=sub.add_parser(name); item.add_argument("--admission",type=Path,required=True); item.add_argument("--known-hosts",type=Path,required=True); item.add_argument("--pve-public-key",type=Path,required=True); item.add_argument("--guest-public-key",type=Path,required=True)
 ...
 for attr in ("admission","known_hosts","pve_public_key","guest_public_key","output_dir","plan","receipt"):
