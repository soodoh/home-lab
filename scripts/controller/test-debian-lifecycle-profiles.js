...
// Real Ansible source evaluation with mocked command endpoints, not a full site
// run. The retired legacy Compose production role is intentionally absent.
assert.equal(site.roles.some((item) => item.role === "compose"), false);
const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "debian-base-"));
