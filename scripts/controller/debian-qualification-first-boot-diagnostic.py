...
def target(args):
 ...
 if value.get("admitted") is not True or value.get("admission_mode") not in ("fresh","expired-offline-diagnostic") or value.get("target_id")!="production-pve-vm9900-qualification": fail("target")
 return value
def stopped(path,target,state,args):
 ...
 current,_=load_canonical_object(args.admission,"current diagnostic admission"); lineage=value.get("admission_sha256")==target["isolation_attestation_sha256"]
 if not lineage:
  if target.get("admission_mode")!="fresh": fail("historical-lineage-requires-fresh-admission")
  prior,prior_raw=load_canonical_object(args.prior_admission,"historical diagnostic admission"); prior_stable={key:item for key,item in prior.items() if key not in ("observed_at","expires_at")}; current_stable={key:item for key,item in current.items() if key not in ("observed_at","expires_at")}; lineage=sha(prior_raw)==value.get("admission_sha256") and prior_stable==current_stable
 if set(value)!=required or value.get("format")!="home-lab-debian-qualification-stop-receipt-v1" or value.get("operation")!="stop" or not lineage or value.get("target_id")!=target["target_id"] or value.get("resources")!=resources or value.get("vmid")!=9900 or value.get("vm_started") is not False or value.get("state_sha256")!=sha(load_protected_bytes(state,"qualification state")): fail("stopped-receipt")
...
def main():
 ...
 for name in ("plan","apply"):
  item=sub.add_parser(name); item.add_argument("--admission",type=Path,required=True); item.add_argument("--prior-admission",type=Path,required=True); item.add_argument("--known-hosts",type=Path,required=True); item.add_argument("--stopped-receipt",type=Path,required=True); item.add_argument("--capability-log",type=Path,required=True); item.add_argument("--output-dir",type=Path,required=True)
