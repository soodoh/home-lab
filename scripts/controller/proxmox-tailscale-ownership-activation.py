#!/usr/bin/env python3
"""Build and apply one no-mutation Proxmox Tailscale-node ownership receipt."""
...
OUTPUT = ROOT / ".local/proxmox-tailscale-ownership-activations"
LOCK = ROOT / ".local/locks/proxmox-tailscale-ownership.lock"
...
ROLE = ROOT / "ansible/roles/proxmox_network_lifecycle/tasks/main.yml"
CONSUMER_ROLE = ROOT / "ansible/roles/proxmox_network_consumers/tasks/main.yml"
...
def clean_pushed_commit() -> str:
    ...
    if commit != git("rev-parse", "origin/main") or git("status", "--porcelain=v1", "--untracked-files=all"):
        raise SystemExit("NETWORK ownership activation requires clean pushed HEAD")
    return commit


def transferred_contract() -> None:
    script = "const fs=require('node:fs'),{load}=require('js-yaml');const v=load(fs.readFileSync(process.argv[1],'utf8'));process.stdout.write(JSON.stringify(v.lifecycle.hosts.proxmox.domain_handoffs.tailscale_node));"
    ...
    if result.returncode or result.stderr or json.loads(result.stdout) != expected:
        raise SystemExit("Tailscale node ownership is not transferred to Ansible")


def observe() -> dict:
    result = subprocess.run((*SSH, "observe tailscale-lifecycle"), capture_output=True, timeout=120)
    if result.returncode or result.stderr:
        raise SystemExit("NETWORK ownership observation failed")
    value = json.loads(result.stdout)
    if value.get("format") != "home-lab-proxmox-tailscale-ownership-observation-v1" or \
        ...
        raise SystemExit("NETWORK ownership parity is incomplete")
    return value


def consumer_parity() -> None:
    result = subprocess.run(("ansible-playbook", "-i", "inventory/production.yml", "playbooks/proxmox-network-plan.yml",
    ...
    if result.returncode or result.stderr or \
        ...
        raise SystemExit("storage consumer parity check failed")
...
def build() -> tuple[Path, str]:
    ...
    value = {
        ...
        "format": "home-lab-proxmox-tailscale-ownership-activation-v1", "host_key_fingerprint": FINGERPRINT,
        "inventory_sha256": file_sha(INVENTORY), "playbook_sha256": file_sha(PLAYBOOK),
        "protected_values_exported": False, "role_sha256": file_sha(ROLE),
...
def load(path: Path) -> tuple[dict, bytes, str]:
    ...
    if not stat.S_ISREG(metadata.st_mode) or stat.S_ISLNK(metadata.st_mode) or metadata.st_uid != os.getuid() or \
        ...
        raise SystemExit("NETWORK ownership activation metadata differs")
...
def controller_lock() -> int:
    ...
    try:
        ...
        os.close(descriptor)
        raise SystemExit("NETWORK ownership controller lock is active") from error
    return descriptor


def apply(path: Path) -> None:
    value, raw, digest = load(path)
    expected = f"apply-proxmox-tailscale-ownership-{digest}"
    if os.environ.get("PROXMOX_TAILSCALE_OWNERSHIP_APPLY_CONFIRMED") != expected:
    ...
    bindings = {"commit": clean_pushed_commit(), "contract_sha256": file_sha(CONTRACT),
                ...
                "role_sha256": file_sha(ROLE), "consumer_role_sha256": file_sha(CONSUMER_ROLE),
                "playbook_sha256": file_sha(PLAYBOOK), "transport_sha256": file_sha(TRANSPORT),
    ...
    if any(value.get(key) != expected_value for key, expected_value in bindings.items()) or \
        ...
        raise SystemExit("NETWORK ownership activation binding or freshness differs")
    transferred_contract()
    consumer_parity()
    if observe() != value.get("before"):
        raise SystemExit("NETWORK ownership observation changed after planning")
    descriptor = controller_lock()
    try:
        staged = subprocess.run((*SSH, f"stage tailscale-ownership {digest}"), input=raw, capture_output=True, timeout=120)
        if staged.returncode or staged.stderr or json.loads(staged.stdout) != {"staged": True}:
            raise SystemExit("NETWORK ownership staging failed")
        result = subprocess.run((*SSH, f"apply tailscale-ownership {digest}"), capture_output=True, timeout=300)
    finally:
        os.close(descriptor)
    if result.returncode or result.stderr:
        raise SystemExit("NETWORK ownership activation failed")
    output = json.loads(result.stdout)
    if output.get("tailscale_ownership_transaction") not in {"committed", "already-committed"} or \
            output.get("changed") is not False or output.get("plan_sha256") != digest:
        raise SystemExit("NETWORK ownership activation result differs")
