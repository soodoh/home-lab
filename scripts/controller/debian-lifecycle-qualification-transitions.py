...
def prior(args,operation,target):
 ...
 required={"admission_sha256","commit","format","operation","plan_sha256","resources","snippet_receipt_sha256","state_sha256","target_id","version","vm_started","vmid"} | ({"prior_receipt_sha256"} if prior_identity else set()) | ({"snippet_sha256"} if operation=="destroy" else set())
 identities=("admission_sha256","plan_sha256","snippet_receipt_sha256","state_sha256")+(("prior_receipt_sha256",) if prior_identity else ())+(("snippet_sha256",) if operation=="destroy" else ())
...
def manifest(args,operation,target,prior_sha,snippet_sha):
 ...
 ttl=dt.timedelta(hours=4) if operation=="repair-network" else dt.timedelta(minutes=30)
