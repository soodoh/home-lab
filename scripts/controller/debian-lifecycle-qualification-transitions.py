...
def plan(args,operation):
 ...
 try:
  ...
  binary=run/f"{operation}.tfplan"; shown=run/f"{operation}.json"; command=["tofu",f"-chdir={common.TF_ROOT}","plan","-input=false","-lock=true",*common.variables(target,key),"-var=start_qualification=true"]
  ...
  resources=inspect(json.loads(shown.read_text()),operation,target); created=common.now(); manifest={"actionable":True,"admission_sha256":target["isolation_attestation_sha256"],"api_ca_sha256":target["api_ca_sha256"],"apply_principal":target["apply_principal"],"authorized":False,"automatic_apply":False,"commit":commit,"created_at":created.isoformat().replace("+00:00","Z"),"endpoint":target["endpoint"],"expires_at":(created+dt.timedelta(minutes=30)).isoformat().replace("+00:00","Z"),"format":"home-lab-debian-qualification-transition-plan-v1","node_name":target["node_name"],"operation":operation,"plan_json_sha256":"","plan_principal":target["plan_principal"],"plan_sha256":"","prior_receipt_sha256":prior_sha,"resources":resources,"snippet_receipt_sha256":verified["snippet_receipt_sha256"],"state_sha256":state_before,"target_id":target["target_id"],"version":1,"vmid":9900}; publish(output,run,binary,shown,manifest)
...
def apply(args,operation):
 ...
 try:
  ...
  fresh=common.snippet(args)
  if fresh.get("snippet_receipt_sha256")!=verified.get("snippet_receipt_sha256") or fresh.get("snippet_receipt_sha256")!=value.get("snippet_receipt_sha256"): fail("snippet-drift")
