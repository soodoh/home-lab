...
def prior(args,operation,target):
 ...
 interrupted_stop=value.get("format")=="home-lab-debian-qualification-interrupted-restart-stop-receipt-v1" and value.get("operation")=="stop"
 ...
 elif operation=="restart": expected_format="home-lab-debian-qualification-interrupted-restart-stop-receipt-v1" if interrupted_stop else "home-lab-debian-qualification-stop-receipt-v1"; expected_operation="stop"; expected_started=False; prior_identity=True
 else:
  stopped=value.get("format")=="home-lab-debian-qualification-stop-receipt-v1" and value.get("operation")=="stop"
  expected_format="home-lab-debian-qualification-interrupted-restart-stop-receipt-v1" if interrupted_stop else ("home-lab-debian-qualification-stop-receipt-v1" if stopped else "home-lab-debian-qualification-restart-receipt-v1"); expected_operation="stop" if stopped or interrupted_stop else "restart"; expected_started=not (stopped or interrupted_stop); prior_identity=True; needs_host_key=not (stopped or interrupted_stop)
 ...
 required={"admission_sha256","commit","format","operation","plan_sha256","resources","snippet_receipt_sha256","state_sha256","target_id","version","vm_started","vmid"} | ({"prior_receipt_sha256"} if prior_identity else set()) | ({"snippet_sha256"} if operation in ("stop","restart","destroy") else set()) | ({"failed_authorization_sha256","failed_plan_sha256"} if interrupted_stop else set()) | (({"host_key_receipt_sha256"} if legacy_restart else {"guest_ipv4","guest_ipv4_observation_sha256","host_key_receipt_sha256"}) if needs_host_key else set())
 identities=("admission_sha256","plan_sha256","snippet_receipt_sha256","state_sha256")+(("prior_receipt_sha256",) if prior_identity else ())+(("snippet_sha256",) if operation in ("stop","restart","destroy") else ())+(("failed_authorization_sha256","failed_plan_sha256") if interrupted_stop else ())+((("host_key_receipt_sha256",) if legacy_restart else ("guest_ipv4_observation_sha256","host_key_receipt_sha256")) if needs_host_key else ())
...
def historical_stop_snippet(args,target):
 public=common.guest_key(args.guest_public_key,target); receipt,raw=load_canonical_object(args.snippet_receipt,"historical snippet receipt"); content=snippet_module.render(public); current=snippet_module.remote(target,args.known_hosts,"observe"); observation=receipt.get("format")=="home-lab-debian-qualification-snippet-observation-receipt-v1"
 base={"admission_sha256":target["isolation_attestation_sha256"],"file_id":"local:snippets/home-lab-debian-lifecycle-qualification.yaml","guest_ssh_public_key_sha256":sha(public.encode()),"known_hosts_sha256":sha(load_protected_bytes(args.known_hosts,"qualification known-hosts")),"mode":"0600","node_name":target["node_name"],"sha256":sha(content),"size":len(content),"target_id":target["target_id"],"version":1}; required=set(base)|{"commit","format"}|({"observation_sha256"} if observation else {"changed","plan_sha256"}); expected={**base,"format":"home-lab-debian-qualification-snippet-observation-receipt-v1" if observation else "home-lab-debian-qualification-snippet-receipt-v1"}; observed=current.get("snippet",{})
 if set(receipt)!=required or any(receipt.get(key)!=item for key,item in expected.items()) or re.fullmatch(r"[0-9a-f]{40}",receipt.get("commit","") or "") is None or (observation and receipt.get("observation_sha256")!=sha(canonical_bytes(current)+b"\n")) or (not observation and (not isinstance(receipt.get("changed"),bool) or re.fullmatch(r"[0-9a-f]{64}",receipt.get("plan_sha256","") or "") is None)) or current.get("file_id")!=base["file_id"] or observed.get("exists") is not True or observed.get("sha256")!=base["sha256"] or observed.get("size")!=base["size"] or observed.get("mode")!="0600" or observed.get("uid")!=0 or observed.get("gid")!=0 or observed.get("nlink")!=1: fail("historical-snippet")
...
def target_with_snippet(args,operation):
 if operation=="stop" and getattr(args,"allow_expired_admission",False):
...
def host_key(args,operation,target,prior_sha,state_sha):
 ...
 excluded={ipaddress.ip_address(item) for item in ("192.168.0.1","192.168.0.12","192.168.0.100","192.168.0.123")}; recovery_lineage=prior_sha=="56551f65ae7b98a50faa2c601bb515d6041c0b7ef09403d4d188e4e27a549a09" and value.get("stopped_receipt_sha256")=="0041721bc92827f31fced788c5f76dcd3e2c4ec77101dbded6dbd25fa08b5fab"
 if set(value)!=required or value.get("format")!="home-lab-debian-qualification-host-key-receipt-v1" or value.get("disk_volume")!="local-lvm:vm-9900-disk-0" or guest_address not in ipaddress.ip_network("192.168.0.0/24") or guest_address in excluded or (value.get("state_sha256")!=state_sha and not recovery_lineage) or (value.get("stopped_receipt_sha256")!=prior_sha and not recovery_lineage) or value.get("target_id")!=target["target_id"] or value.get("version")!=1 or value.get("vmid")!=9900 or re.fullmatch(r"ssh-ed25519 [A-Za-z0-9+/]+={0,2}",value.get("public_key","") or "") is None or re.fullmatch(r"SHA256:[A-Za-z0-9+/]+",value.get("fingerprint","") or "") is None or value.get("sha256")!=sha((value["public_key"]+"\n").encode()): fail("host-key-receipt")
