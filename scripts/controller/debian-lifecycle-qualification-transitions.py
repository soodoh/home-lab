...
def prior(args,operation,target):
 value,raw=load_canonical_object(args.prior_receipt,"qualification prior receipt"); needs_host_key=False; recovery_stop=False
 ...
 elif operation=="restart": recovery_stop=value.get("format")=="home-lab-debian-qualification-interrupted-restart-stop-receipt-v1" and value.get("operation")=="stop"; expected_format="home-lab-debian-qualification-interrupted-restart-stop-receipt-v1" if recovery_stop else "home-lab-debian-qualification-stop-receipt-v1"; expected_operation="stop"; expected_started=False; prior_identity=True
 else:
 ...
 required={"admission_sha256","commit","format","operation","plan_sha256","resources","snippet_receipt_sha256","state_sha256","target_id","version","vm_started","vmid"} | ({"prior_receipt_sha256"} if prior_identity else set()) | ({"snippet_sha256"} if operation in ("stop","restart","destroy") else set()) | ({"failed_authorization_sha256","failed_plan_sha256"} if recovery_stop else set()) | (({"host_key_receipt_sha256"} if legacy_restart else {"guest_ipv4","guest_ipv4_observation_sha256","host_key_receipt_sha256"}) if needs_host_key else set())
 identities=("admission_sha256","plan_sha256","snippet_receipt_sha256","state_sha256")+(("prior_receipt_sha256",) if prior_identity else ())+(("snippet_sha256",) if operation in ("stop","restart","destroy") else ())+(("failed_authorization_sha256","failed_plan_sha256") if recovery_stop else ())+((("host_key_receipt_sha256",) if legacy_restart else ("guest_ipv4_observation_sha256","host_key_receipt_sha256")) if needs_host_key else ())
