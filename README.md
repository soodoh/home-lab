...
The authoritative desired-state contract is [`infrastructure/contract/home-lab.yml`](infrastructure/contract/home-lab.yml). OpenTofu owns isolated infrastructure roots, controller-side Nix owns Proxmox host configuration, Ansible owns the Debian Docker host, and Compose owns applications. The infrastructure controller supports steady reconciliation only; use the guarded workflow documented in [`docs/local-controller.md`](docs/local-controller.md).
